...
class AuthenticationInterceptor @Inject constructor(private val db: AppDatabase): Interceptor {
    override fun intercept(chain: Interceptor.Chain): Response {
        val original = chain.request()
        val originalUrl = original.url

        val sessionId = runBlocking { db.configurationDao().getConfiguration()?.sessionId } ?: return chain.proceed(original)
